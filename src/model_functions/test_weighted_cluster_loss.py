...
# encode to one hot to try the non-sparse layer
train_labels = np.squeeze(one_hot[train_labels])
test_labels = np.squeeze(one_hot[test_labels])
...
labels = tf.keras.Input(shape=(10, ), dtype='int32')
...
# scl = SparseClusterLayer(num_classes=10, class_weight=class_weights, name='cluster')  # -> modify label Input layer!
scl = ClusterLayer(num_classes=10, class_weight=class_weights, name='cluster')  # -> modify label Input layer!
...
# test with tensorflow SparseCategoricalCrossentropy
# model.compile(optimizer=tf.keras.optimizers.SGD(learning_rate=0.001, momentum=0.9),
